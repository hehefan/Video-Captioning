...
class Seq2Seq(object):
  def __init__(self, num_units, use_lstm, epsilon, max_computation, encoder_max_sequence_length, decoder_max_sentence_length, feature_size, vocab_size, learning_rate, learning_rate_decay_factor, time_penalty, max_gradient_norm, forward_only=False):
    ...
    encoder = ACTCell(encoder, epsilon, max_computation)
    ...
    with variable_scope.variable_scope("embedding_rnn_seq2seq") as scope:
      _, self.encoder_state = tf.nn.rnn(cell=encoder, inputs=self.encoder_inputs, dtype=tf.float32)
      if forward_only:
        self.outputs, self.decoder_state = tf.nn.seq2seq.embedding_rnn_decoder(self.decoder_inputs, self.encoder_state, decoder, vocab_size, num_units, feed_previous=True, update_embedding_for_previous=False)
      else:
        self.outputs, self.decoder_state = tf.nn.seq2seq.embedding_rnn_decoder(self.decoder_inputs, self.encoder_state, decoder, vocab_size, num_units, feed_previous=False)
    self.remainders = encoder.ACT_remainder
    self.iterations = encoder.ACT_iterations
    # Training outputs and losses.
    if not forward_only:
      batch_size = array_ops.shape(self.encoder_inputs[0])[0]
      self.loss1 = tf.nn.seq2seq.sequence_loss(self.outputs, self.targets, self.target_weights)
      self.loss2 = encoder.calculate_ponder_cost() * time_penalty
  ...
  def step(self, session, encoder_inputs, decoder_inputs, target_weights, forward_only):
    ...
    if forward_only:
      output_feed = [self.outputs, self.remainders, self.iterations]
      outputs = session.run(output_feed, input_feed)
      return outputs[0], outputs[1], outputs[2]
    else:
      output_feed = [self.loss1, self.loss2, self.remainders, self.iterations, self.update]
      outputs = session.run(output_feed, input_feed)
