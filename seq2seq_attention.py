...
tf.app.flags.DEFINE_integer("decoder_max_sentence_length", 25, "Max length of decoder sentence.")
tf.app.flags.DEFINE_boolean("decoder_use_lstm", False, "LSTM or GRU for decoder.")
tf.app.flags.DEFINE_string("checkpoint_dir", "CheckPoint_Att", "Checkpoint directory.")
...
def embedding_rnn_seq2seq(encoder_inputs, decoder_inputs, encoder_cell, decoder_cell, num_decoder_symbols, decoder_size, feed_previous=False, dtype=None, scope=None):
  with variable_scope.variable_scope(scope or "embedding_rnn_seq2seq") as scope:
    #encoder_cell = tf.nn.rnn_cell.OutputProjectionWrapper(encoder_cell, 512)
    encoder_outputs, state = rnn.rnn(encoder_cell, encoder_inputs, dtype=tf.float32)
    ...
    decoder_cell = tf.nn.rnn_cell.OutputProjectionWrapper(decoder_cell, num_decoder_symbols)
    if feed_previous:
      return tf.nn.seq2seq.embedding_attention_decoder(decoder_inputs=decoder_inputs, initial_state=state, attention_states=attention_states, cell=decoder_cell, num_symbols=num_decoder_symbols, embedding_size=decoder_size, num_heads=1, feed_previous=True, update_embedding_for_previous=False)
    else:
      return tf.nn.seq2seq.embedding_attention_decoder(decoder_inputs=decoder_inputs, initial_state=state, attention_states=attention_states, cell=decoder_cell, num_symbols=num_decoder_symbols, embedding_size=decoder_size, num_heads=1, feed_previous=False)
