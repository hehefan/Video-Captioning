  ...
  with tf.Session() as sess:
    ...
    while current_step <= 10000:
      ...
      for start,end in zip(range(0, len(sentence), FLAGS.batch_size), range(FLAGS.batch_size, len(sentence), FLAGS.batch_size)):
        feature_inputs, feature_lengths, batch_decoder_inputs, batch_weights = model.get_batch(feature, sentence[start:end], length)
        step_loss = model.step(sess, feature_inputs, feature_lengths, batch_decoder_inputs, batch_weights, False)
        current_step += 1
