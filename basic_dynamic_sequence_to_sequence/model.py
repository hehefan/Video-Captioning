...
class Seq2Seq(object):
  def __init__(self, num_units, use_lstm, encoder_max_sequence_length, decoder_max_sentence_length, feature_size, vocab_size, learning_rate, learning_rate_decay_factor, max_gradient_norm, forward_only=False):
    ...
    with variable_scope.variable_scope("embedding_rnn_seq2seq") as scope:
      encoder_outputs, self.encoder_state = rnn.rnn(cell=encoder, inputs=self.encoder_inputs, sequence_length=self.encoder_lengths, dtype=tf.float32)
      if forward_only:
  ...
  def step(self, session, encoder_inputs, encoder_lengths, decoder_inputs, target_weights, forward_only):
    ...
    input_feed[self.encoder_lengths.name] = encoder_lengths
  ...
  # For beam search.
  def state(self, session, encoder_inputs, lengths):
    input_feed = {}
    for l in xrange(self.encoder_max_sequence_length):
      input_feed[self.encoder_inputs[l].name] = encoder_inputs[l]
    input_feed[self.encoder_lengths.name] = lengths
